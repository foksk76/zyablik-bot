# Конфигурация бота — runbook (ADR-0045/0046)

Документ описывает работу с файлом конфигурации `zyablik.config.json` как
источником правды для управляемых настроек bot-platform: миграцию с `.env`,
ежедневную правку, apply/rollback, авто-откат и восстановление.

## Статус

```text
Runbook / Draft
```

## Назначение

С Sprint 37-38 файл `zyablik.config.json` — основной способ конфигурации
управляемых настроек (ADR-0045). В `.env` остаются:

```text
ZYABLIK_CONFIG            — путь к конфиг-файлу (bootstrap)
NODE_EXTRA_CA_CERTS       — сертификаты TLS для исходящих HTTPS
MAX_BOT_TOKEN             — секрет: $VAR-ссылка в файле
METRICS_API_KEY           — секрет: $VAR-ссылка в файле
SESSION_SECRET            — секрет: $VAR-ссылка в файле
IDP_CLIENT_SECRET         — секрет (IdP-регистрация, остаётся в env)
MAX_API_URL               — неизменяемая база
IDP_ISSUER / IDP_AUDIENCE / IDP_CLIENT_ID / IDP_REDIRECT_URI — неизменяемая база
```

Управляемые настройки (`bot.*`, `queue.*`, `ingress.*`, `monitor.*`,
`plugins.<name>.*`) читаются из файла. Секреты в файле — только `$VAR`-ссылки
(формат `^$[A-Z0-9_]+$`), значения подставляются из env при старте.

## 1. Миграция с .env: --generate-config

Первый запуск на новом стенде или перевод существующего `.env`-стенда:

```bash
# Просмотр, что будет записано (без записи):
node src/bot-platform/app.js --generate-config --dry-run

# Запись первого zyablik.config.json (не перезаписывает существующий):
node src/bot-platform/app.js --generate-config
```

Файл создаётся в `./config/zyablik.config.json` (если `ZYABLIK_CONFIG` не
задан) или по `ZYABLIK_CONFIG`. Секреты записываются как `$VAR`-ссылки.

Для docker-стенда:

```bash
docker compose run --rm zyablik node src/bot-platform/app.js --generate-config
```

## 2. Структура файла

```json
{
  "version": 1,
  "bot": {
    "logLevel": "info",
    "maxTransportMode": "long_polling",
    "httpProxy": "",
    "maxPollLimit": 100,
    "maxPollTimeoutSeconds": 30,
    "maxPollTypes": ["message_created", "bot_started", "bot_added"],
    "rateLimitEnabled": true,
    "rateLimitGlobal": 25,
    "rateLimitRecipient": 5,
    "logAudit": false,
    "logTrace": true,
    "maxBotToken": "$MAX_BOT_TOKEN"
  },
  "queue": {
    "enabled": false,
    "maxAttempts": 5,
    "intervalMs": 5000,
    "batchSize": 10,
    "backoffBase": 2,
    "backoffMax": 300,
    "processingTtlSeconds": 300
  },
  "ingress": {
    "enabled": false,
    "port": 8443,
    "jwtClaimName": "entitlements",
    "jwtClaimValue": "zabbix"
  },
  "monitor": {
    "enabled": true,
    "port": 9000,
    "metricsApiKey": "$METRICS_API_KEY",
    "sessionSecret": "$SESSION_SECRET",
    "authRateLimit": true,
    "authRateLimitMax": 20,
    "authRateLimitWindowMs": 60000,
    "authRateConcurrency": 5,
    "idpRelaxSsrf": false,
    "idpRequireDiscovery": true
  },
  "plugins": {}
}
```

Схема и версия файла: `version: 1` (ADR-0045, `config-migrations.js`).
Поля `plugins.<name>.*` — по `configSchema` плагина (ADR-0046).

## 3. Ежедневная правка через web UI (рекомендуется)

1. Открыть dashboard: `https://<stand-host>/` (или `http://localhost:9000/`).
2. Перейти в раздел **Настройки**.
3. Изменить поля в форме (форма генерируется из merged-схемы, ADR-0046).
4. Нажать **Сохранить (staged)** — изменения записываются в
   `zyablik.config.staged.json`, показывается diff.
5. Нажать **Применить (рестарт)** — `POST /api/config/apply` (202); процесс
   рестартует. Статус опрашивается автоматически:
   `pending → confirmed` (успех) или `rolled_back` (авто-откат).

> **Контракт рестарта (L3, review R7):** `202 Accepted` отдаётся до
> фактического рестарта, а сам рестарт обязан завершать процесс
> асинхронно — после отправки ответа и установки `pending`. Синхронный
> рестарт внутри Apply потерял бы ответ и статус `pending`. Сейчас на стенде
> рестарт делегирован наружу (systemd `Restart=on-failure` + сигнал себе,
> см. §5) и контракт соблюдается.

Секреты в форме не редактируются — показывается только статус
«задан / не задан».

> **Предупреждения загрузки (R11-L3, review PR #23):** если при старте
> `loadConfig` нашёл неопознанные ключи или устаревшие `$VAR`-значения,
> предупреждения накапливаются в `createCore.configWarnings` и показываются
> баннером на странице «Настройки» (из `GET /api/config`), а не только в
> логах процесса. Баннер исчезает после исправления конфига и рестарта.

Поля `plugins.<name>.*` плагина без `configSchema` (schemaless-ветки) в
API-ответах маскируются целиком (значения не раскрываются), а в staged
при «Сохранить» такие ключи не переносятся — их не редактируют из UI.

> **Необъявленные ключи (F10-L1, review R10):** ключи, отсутствующие в
> схеме (системная секция или `plugins.<name>.*`), в UI не редактируются:
> строки в API-ответах маскируются до статуса, объекты/массивы/числа
> отбрасываются. При импорте API отвечает списком `warnings` с такими
> ключами, которых **нет в активном конфиге** (в том числе `$VAR`-секреты) —
> UI показывает предупреждение. Пока staged не пересохранён, значения
> целы (Apply читает файл напрямую); но **«Сохранить (staged)» из формы
> такие ключи молча потеряет** — `mergePreservedSecrets` возвращает только
> ключи, уже присутствующие в активном конфиге. Чтобы сохранить их, верните
> ключ в активный конфиг или правите файл вручную (§4).

## 4. Ручная правка файла (для продвинутых)

```bash
nano config/zyablik.config.json
# затем рестарт:
node src/bot-platform/app.js
```

Валидация при старте: JSON, `version`, схема, литеральные секреты (reject),
`$VAR`-резолв. Ошибки — в stdout/stderr; файл карантинится в
`zyablik.config.<ts>.bad.json` (`<ts>` — уникальная временная метка, чтобы
повторный карантин не затирал предыдущие улики), восстанавливается `.lkg`.

## 5. Apply / Rollback из API или UI

- **Apply** — записывает активный файл, копирует его в
  `zyablik.config.json.lkg`, ставит pending-маркер
  `zyablik.config.json.pending` (хеш), рестартует. По выходу на ready маркер
  снимается → конфиг закреплён (confirmed).
- **Rollback (ручной)** — восстанавливает `.lkg`, снимает pending-маркер,
  рестартует.

CLI:

```bash
node src/bot-platform/app.js --rollback-config
node src/bot-platform/app.js --rollback-config /path/to/zyablik.config.json
```

## 6. Авто-откат (crash recovery)

Стартовый детектор (ADR-0045) при старте проверяет два условия:

1. **Валидационный отказ** — файл не прошёл схему/version/$VAR: карантин в
   `.bad.json`, восстановление `.lkg`, старт с lkg.
2. **Pending-маркер без подтверждения** — Apply был, но процесс упал до
   ready: откат к `.lkg`, маркер снят, неприменённый конфиг сохраняется в
   карантин (`.bad.json`) как улика. Окно `StartupWait` отсчитывается от
   последнего старта (`lastBoot`); для первого boot после Apply — от времени
   Apply, но только если рестарт инициировал сам Apply (`restartInitiated` в
   маркере). Если рестарт ручной — первый boot продолжается без окна
   (задержка между Apply и рестартом оператора — не признак краша), а защита
   от crash-loop работает через счётчик попыток (`boots`).

Если и `.lkg` невалиден — процесс отказывается стартовать (fail loudly),
рестарт-политика (systemd `Restart=always`, docker `unless-stopped`)
ограничена, чтобы не было crash-loop.

После такого авто-отката/карантина dashboard показывает статус
`rolled_back`/`quarantine` (banner «Конфигурация откачена»): результат
стартового детектора пробрасывается в `/api/config/status` через
`recoveryState` (M6), т.е. статус виден даже после рестарта процесса.

**Зависание boot'а при сетевом сбое (M1).** В live-режиме `start()` ждёт
первый успешный long-polling цикл (`firstTick`) ограниченно: по умолчанию
60 секунд (`DEFAULT_FIRST_TICK_TIMEOUT_MS`, кнопка `firstTickTimeoutMs` в
live-опциях). Если первый успешный poll не случился в пределах лимита
(неверный `MAX_BOT_TOKEN`/`MAX_API_URL`, сеть недоступна) — процесс логирует
ошибку, останавливает сервисы (ingress/worker/queue-store) и завершается с
ненулевым кодом. Рестарт-политика (systemd `Restart=on-failure`, docker
`unless-stopped`) перезапускает процесс; pending-маркер при этом не
подтверждён, поэтому после истечения окна `StartupWait` или счётчика
`boots` (`maxStartupAttempts`, по умолчанию 5) следующий boot откатывает
конфиг к `.lkg`. Так авто-откат покрывает и сетевые сбои конфигурации, а не
только краш до ready. Подтверждение (`confirm`) выполняется только после
успешного `firstTick`.

> **Трейд-офф авто-отката (R13-M2, review PR #23).** Авто-откат защищает от
> незавершённого Apply, но платит за это: конфиг считается подтверждённым
> только после успешного `confirm()` (ready) — то есть до момента, пока
> оператор не увидит в UI статус `confirmed`, процесс может (при падении в
> окне `StartupWait`) откатиться к `.lkg` без участия человека. Конкретно:
> - **Окно подтверждения ограничено.** `startupWaitMs` (30 с по умолчанию)
>   отсчитывается от `lastBoot`. Если boot реально занимает дольше окна и
>   процесс падает ПОСЛЕ истечения окна — следующий старт не откатит конфиг
>   (окно исчерпано), но и не подтвердит его (confirm не выполнен);
>   подтверждение произойдёт на следующем boot'е, если `lastBoot` обновился.
> - **Медленный boot может быть откачен.** Если boot затягивается дольше
>   `startupWaitMs` и при этом падает — следующий boot откатит **валидный**
>   конфиг, который просто не успел подтвердиться. Это плата за автоматику;
>   ручной выход — большой запас по времени подтверждения на стенде.
> - **Баннер pending честнее таймера.** UI-обратный отсчёт
>   `pendingRemainingMs` считается от той же базы, что и откат (`lastBoot`),
>   а статус `confirmed` (или `rolled_back`) приходит после фактического
>   рестарта и `confirm()`; при медленном рестарте процесс может быть уже
>   перезапущен, а маркер ещё жив — баннер pending остаётся до подтверждения.
>   Это намеренно: авто-откат активен именно до `confirm()`, а не до конца
>   перезапуска.

### Системные ограничители рестартов (R17, review PR #23)

Схема systemd-юнитов (`systemd/zyablik-bot.service`,
`systemd/zyablik-bot-live.service`) настроена в пару с детектором:

```ini
Restart=always
RestartSec=5
StartLimitIntervalSec=120
StartLimitBurst=5
```

`Restart=always` держит сервис живым при любом ненулевом выходе (в т.ч.
намеренном падении после Apply). `StartLimitIntervalSec=120` /
`StartLimitBurst=5` — страховка от бесконечного crash-loop вне детектора:
systemd перестаёт поднимать юнит, если за 120 секунд было больше 5 стартов.

Согласованность с детектором (ADR-0045):

```text
детектор maxStartupAttempts = 5 (boots-счётчик pending-маркера)
systemd  StartLimitBurst     = 5 (попыток за StartLimitIntervalSec)

детектор startupWaitMs  = 30_000 (окно StartupWait от lastBoot)
systemd  RestartSec     = 5 (пауза между рестартами)
         StartLimitIntervalSec = 120 (окно подсчёта попыток)
```

- **Счётчик детектора сбрасывается только подтверждением** (`confirm()`);
  счётчик systemd сбрасывается после `StartLimitIntervalSec` без падений.
- **Стартовые попытки в стиле 120 с/5** (примерно 5 рестартов по
  `RestartSec=5`) укладываются в окно, если каждый boot быстрый. Если boot
  занимает время, близкое к `startupWaitMs` (30 с), systemd может счесть
  падение за пределами burst-окна — но детектор продолжит отсчёт своих 5
  попыток (их «окно» — до подтверждения, не 120 с). Для одинакового
  поведения двух счётчиков важно, чтобы boot успевал подтвердиться быстрее,
  чем накапливаются 5 попыток; практически это обеспечивается `RestartSec=5`
  и диагностикой M1 (`firstTickTimeoutMs` — явное падение, а не зависание).
- **Если systemd сдался** (start-limit-hit) — юнит требует ручного
  `systemctl reset-failed` + `systemctl start`. Детектор при этом бездействует
  (процесс не стартует), конфиг остаётся на диске, `.lkg` цел; следующий
  ручной старт применит обычную логику (валидационный отказ → карантин +
  восстановление из `.lkg`, pending без подтверждения → авто-откат).

## 7. Восстановление из резервной копии

```bash
# Свежий дамп (без литеральных секретов — $VAR-ссылки):
#  GET /api/config/export (UI: кнопка «Экспорт»)

# Восстановление на другом стенде:
cp backup.json config/zyablik.config.json
node src/bot-platform/app.js   # валидация + старт
```

Экспорт маскирует литеральные секреты объявленных полей (R5-L1, review PR
#23): если в активном файле оказался литерал (ручная правка вне
apply-потока), в дампе он заменяется на `''` — бэкап не становится каналом
утечки. `$VAR`-ссылки сохраняются без изменений.

При импорте файла с литеральным секретом API/UI вернёт 400 со списком полей —
заменить на `$VAR`-ссылку.

## 8. Права на ./config

Каталог `./config` — writable для процесса бота (Stage/Apply/Rollback пишут
файлы и маркеры рядом с активным конфигом). В docker volume
`./config` монтируется как host-каталог (docker-compose.yml); секреты в файл
не пишутся — только `$VAR`-ссылки. Резервное копирование каталога
(включая `.lkg`) достаточно для восстановления конфигурации.

## 9. Служебные файлы

```text
config/zyablik.config.json           — активный конфиг
config/zyablik.config.json.lkg       — last-known-good (перед apply)
config/zyablik.config.json.pending   — pending-маркер (хеш применяемого)
config/zyablik.config.json.staged    — staged-снапшот (редактирование)
config/zyablik.config.<ts>.bad.json  — карантин невалидного файла
                                       (<ts> — уникальная временная метка)
```
