...
Поведение платформы MAX: чтобы получать события из групповых чатов и каналов, бот должен быть **администратором** чата. Это подтверждено официальной документацией MAX Bot API (`https://dev.max.ru/docs-api/methods/GET/updates`). См. `docs/identity-plugin/group-chat-update-delivery.md` для полного анализа с цитатами.
...
Это подтверждает, что transport, токен и сеть работают (200, а не 401 и не ошибка TLS), но MAX Bot API не доставляет боту `message_created` для сообщений в этом чате — потому что бот не администратор. В личном диалоге (dialog) `message_created` доходит и обрабатывается.

Гипотеза про упоминание (`@bot`) или ответ (reply) на сообщение бота **официальной документацией не подтверждена и снята** (см. `docs/identity-plugin/group-chat-update-delivery.md`, раздел «Что НЕ является условием доставки»).
