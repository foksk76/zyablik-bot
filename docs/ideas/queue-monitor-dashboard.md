...
> **Внимание:** Этот pre-ADR idea document устарел. Принятое решение:
> [ADR-0034](../decisions/ADR-0034-queue-monitor-dashboard.md).
> Некоторые детали в этом документе (auth endpoints, dependency list) не
> соответствуют финальному решению. См. ADR-0034 как каноничный источник.
