...
- **Rate-триггеры и свежая линковка**: выражения роста ошибок
  (`last(failed) - last(failed, {$ZYABLIK.BACKLOG_SEC})`) считают дельту
  счётчика за окно. Если внутри окна нет ни одного значения (сразу после
  привязки шаблона к хосту или после разрыва данных длиннее окна),
  `last(..., sec)` возвращает 0 и дельта равна **всему** накопленному
  счётчику — триггер может сработать сразу против порога
  `{$ZYABLIK.MAX_FAILED}`. Это ожидаемое поведение (не баг): после
  накопления окна истории дельта считается корректно. Учитывайте при
  первом включении шаблона на новом хосте.
