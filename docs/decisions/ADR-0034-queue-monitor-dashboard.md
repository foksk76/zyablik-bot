...
> JSONPath в dependent items. **Это целевое состояние:** реализация
> `src/queue-monitor/api/metrics.js` изменяется в Sprint 33 Task 1 (ADR-0043);
> до этого код и тесты возвращают промежуточную форму с префиксом `queue.`.
>
> **История:** первоначальная версия ADR описывала четыре ключа без
> префикса (`delivered`, `failed`, `processing`, `pending`). Промежуточная
> поправка от 2026-08-01 уточнила реализацию — шесть ключей с префиксом
