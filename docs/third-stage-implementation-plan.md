...
- нет реальных идентификаторов и секретов.
...
- токен только из внешней конфигурации;
...
- проверка, что токен не логируется;
...
- описан способ запуска сервиса;
- описан способ запуска тестов;
- описан способ передачи временного webhook endpoint, если требуется интеграционный прогон.
...
- агент не работает с реальными секретами;
...
```text
WSL — удобен для локальной разработки на Windows.
LXC на Proxmox — предпочтителен для более стабильного интеграционного стенда.
```

Окончательный выбор стенда фиксируется после Task 12.4.
...
- агент не получает реальные токены;
- агент не коммитит реальные идентификаторы;
- агент не меняет `src/zabbix-media-type/max-webhook.js` без отдельной задачи;
- агент не добавляет БД, очередь, retry или Zabbix API без нового ADR.

## Definition of Done третьего этапа

- MVP работает на обезличенных fixtures.
- MVP проходит локальные тесты.
- Интеграционный прогон с тестовым ботом выполнен или явно отложен.
- Стенд WSL или LXC проверен.
