...
4. Проверить план работ в `tasks/plan.md`.
5. Выбрать задачу из `tasks/todo.md`.
...
| Task breakdown | Перед началом реализации | Задача в `tasks/todo.md` с критериями и проверкой | `planning-and-task-breakdown` |
...
| Local repository check | После любой правки | Успешный `bash scripts/verify-repo.sh` | `code-review-and-quality` |
...
```text
Repo check              bash scripts/verify-repo.sh
Static docs check       сверка README, docs, tasks, AGENTS
